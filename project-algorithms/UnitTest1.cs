...
namespace Tests
{
    public class Tests
    {
        ...
        {
            /**
             * List to hold distance from source to vertex u, dist[]
             * List to hold if tight, mst[]
             * List to hold predecessor pred[]
             * Init dit[] with inifinity for all vertices except starting node which is 0

             * While not all vertices in mst[]
             * Pick vertex u, which is not in mst[] and has min dist[]
             * Add to mst[]
             * Update dist[] with
             * loop through vertices vi adjacent to u and if dist[u] + edge(uv) < dist[v], dist[v] = dist[u] + edge(uv)
             * 
             */
            var minSpanningTree = CalculateMinimumSpanningTree();
        ...
        private MinSpanningTree CalculateMinimumSpanningTree()
        {
            int[,] graph = new int[,]
            {
                {0, 4, 0, 0, 0, 0, 0, 8, 0},
        ...
        private int GetClosestNode(int[] dist, bool[] sptSet)
        {
